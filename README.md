...
| Garantia de idempotência | ✅ | `externalId` único no banco. Reenvio igual responde `200` com o pedido original; dados diferentes respondem `409`. Validado manualmente com 10 envios simultâneos contra o Postgres (1 × 202, 9 × 200, 1 pedido gravado). Na suíte, um teste simula a corrida de forma determinística e cobre o caminho de recuperação (IntegrityError → responde como reenvio). | `orders/service.py` → `OrderService.receive` |
...
| Testes das principais regras de negócio | ✅ | Validação, idempotência (inclusive o caminho de recuperação da corrida), estados, retentativas, falhas, retomada após queda do worker, reprocessamento, integração com o mock, GraphQL e simulador | `backend/tests/test_orders_*.py`, `test_order_reprocess.py`, `test_graphql_*.py`, `test_order_simulator.py`; `frontend/src/**/*.test.ts(x)` |
...
1. **No recebimento, pelo banco.** A coluna `external_id` é `UNIQUE`. Antes de gravar, a API procura o `externalId`:
   ...
   - duas requisições simultâneas passam juntas pela busca: a constraint barra a segunda no `INSERT`, e ela é respondida como reenvio. Validado manualmente com 10 envios simultâneos contra o Postgres (1 × 202, 9 × 200, 1 pedido gravado). Na suíte, um teste simula a corrida de forma determinística e cobre o caminho de recuperação (IntegrityError → responde como reenvio).
